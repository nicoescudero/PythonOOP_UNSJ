...
from classes.interface import ClassInterface
...
class Lista:
    ...
    def buscarPatente(self,patente):
        ...
        flag = False
        while aux != None:
            flag = aux.setPrecio(patente)
            if flag: aux = None
            else: aux = aux.getSiguiente()
        if flag == False: print('Vehiculo no encontrado')
    def vehiculoEconomico(self):
        aux = self.__comienzo
        car = self.__comienzo
        minimo = car.getPrecio()
        while aux != None:
            precioActual = aux.getPrecio()
            if precioActual < minimo:
                car = aux
                minimo = precioActual
            aux = aux.getSiguiente()
        print(car.getAllData())
    def toJSON(self):
        aux = self.__comienzo
        vehiculos = []
        while aux != None:
